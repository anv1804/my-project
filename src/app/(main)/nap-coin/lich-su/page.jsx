...
import TopupChart from "@/components/features/TopupChart";
    ...
    <div className="w-full max-w-5xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-[var(--color-binance-border)] pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-[var(--color-binance-light)]">Lịch sử nạp tiền</h1>
            <p className="text-sm text-[var(--color-binance-gray)] mt-1">
              Chi tiết các giao dịch nạp tiền vào tài khoản
            </p>
          </div>
        </div>
                  ...
                  return (
                    <tr key={log.id} className="hover:bg-[var(--color-binance-darker)]/40 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right font-bold whitespace-nowrap">
                        <div className={`flex items-center justify-end gap-1 ${log.action === "coin_refund" ? "text-[var(--color-binance-yellow)]" : isPositive ? "text-[var(--color-binance-success)]" : "text-red-400"}`}>
