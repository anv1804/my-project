...
import { Home, ChevronDown, ChevronRight, ChevronLeft, Settings } from "lucide-react";
...
export default function Sidebar() {
  ...
  const toggleGroup = (group) => {
    ...
    // Nếu có link riêng của nhóm thì chuyển hướng luôn
    if (group.href) {
  ...
  return (
    <aside className={cn(
      ...
      <div className="h-16 flex items-center px-4 border-b border-[var(--color-binance-border)] flex-shrink-0 bg-[var(--color-binance-darker)] overflow-hidden">
        <Link href="/" className={cn("flex items-center gap-2 group whitespace-nowrap", isSidebarCollapsed && "mx-auto")}>
      ...
      <nav className="flex-1 overflow-y-auto p-3 flex flex-col gap-2 overflow-x-hidden">
        ...
        <Link 
          ...
          className={cn(
            ...
            isSidebarCollapsed && "justify-center px-0 bg-none"
          )}
        >
          ...
          return (
            <div key={group.id} className="mb-2">
              <button 
                ...
                title={group.title}
                className={cn(
                  "flex items-center justify-between w-full px-3 py-2 text-[14px] font-medium text-[var(--color-binance-gray)] hover:text-[var(--color-binance-light)] transition-colors group cursor-pointer whitespace-nowrap",
                    ...
                    {group.items.map((item, idx) => {
                      ...
                      const itemContent = (
                        <>
                          ...
                          {item.isPro && (
                            <span className="ml-auto text-[9px] font-bold bg-gradient-to-r from-[var(--color-binance-yellow)] to-yellow-500 text-black px-1.5 py-0.5 rounded-[3px] uppercase tracking-wider flex-shrink-0 shadow-sm">
                              PRO
                            </span>
                          )}
                        </>
                      ...
                      if (item.type === "link") {
                        ...
                        return (
                          <Link key={idx} href={item.href} className={cn(baseClass, active ? activeClass : inactiveClass, "w-full overflow-hidden")}>
                            {itemContent}
                          </Link>
                        );
                      }
                      ...
                      if (item.type === "action") {
                        return (
                          <button key={idx} onClick={() => handleAction(item.actionId)} className={cn(baseClass, inactiveClass, "w-full text-left overflow-hidden")}>
                            {itemContent}
                          </button>
                        );
                      }
